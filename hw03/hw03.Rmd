...
The overall maximum and minimum across all continents:
```{r}
...
It looked like Europe, Oceania and Asia had much more spread of data than America and Afria.  To quantify the spread of GDP per capita, the summary statistics table was created below:
```{r}
out3<-
...
These statistics can always be shown visually, using boxplots.
...
In the earlier section, it was shown that the outliers in Asia belonged to Kuwait, from 1952 to 1977.  The plots can be zoomed in by excluding these outliers.  Note that excluding 6 extreme points in Asia did not alter the median and interquartile range of Asia significantly (data not shown).
```{r option3.2}
