...
Even though I have used R for years, I did not use `setwd()`, let alone `getwd()`.  I have defined the **entire path** where the dataset was called from or was to be saved into. To get around the issue of **using back/foward slashes** to define a path, I saved 2 paths: one for MAC and one for PC with comments so whoever runs my code knew which path to select.  Come to think of it, it was rather tedious having to define paths twice.  
...
<br/><br/>
I've extracted a figure from HW02.  On the left hand side, this figure was created for HW02.
```{r}
...
f2<-gapminder %>% ggplot(aes(x=lifeExp))+
  ...
  theme(panel.border = element_blank(), panel.background=element_blank(),
        panel.grid.major.y = element_line(colour='light grey'), panel.grid.minor.y = element_line(colour='light grey'), 
...
```{r}
ggsave("Updated.tiff", width = 10, height = 10, units = "cm")

```


![Loading back the image exported by `ggsave`](./updated.tiff)
